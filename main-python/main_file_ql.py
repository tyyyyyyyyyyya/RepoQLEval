...
class LLMApiClient:
    ...
    def call(self, prompt: str, model: str, use_history: bool = True, py_folder: Union[str, None] = None,
        ...
        if py_folder:
            ...
            if py_files:
                ...
                prompt = f"{prompt}\nThe following are the contents of all Python files in this project：\n{py_content}"  # 在原有prompt基础上加入了全部的py文件内容
                #print(f"这是{prompt}/n")
                #效果显著，在测试中第二次生成就成功了
...
# 1. 生成 QL 查询
    try:
        ...
        ql_position = os.path.join("C:/Users/86183/Desktop/codeql-python/qls-python-211", ql_position)

        client.call(prompt, model="claude-3-haiku-20240307", use_history=True, py_folder=project_dir,
...
def main():
    ...
    for subdir in next(os.walk(root_dir))[1]:
        cat=subdir[:-2]
        prompt = f"Write a CodeQL query in the Python language to test whether {cat} exists in the codebase.Require that if there is a vulnerability in the output results, the file name where the vulnerability is located must be specified in the results."
        prompt1 = f"Write a CodeQL query in the Python language to test whether {cat} exists in the codebase.Require that if there is a vulnerability in the output results, the file name where the vulnerability is located must be specified in the results."
        n = 0
        result = False
